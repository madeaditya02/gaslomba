...
const page = usePage()
const form = useForm({
  // cabang_lomba: props.pendaftaran.cabang_lomba,
  berkas: undefined,
  bukti_pembayaran: undefined,
  kode_anggota: props.pendaftaran.anggota.map(a => a.kode_identitas).slice(1, props.pendaftaran.anggota.length),
  berkas_anggota: props.pendaftaran.anggota.map(a => a.pivot.berkas_identitas).slice(1, props.pendaftaran.anggota.length),
  id_lomba: props.pendaftaran.id_lomba,
})
const selectedCablom = computed(() => {
  return props.lomba.cabang_lomba.find(cablom => cablom.id_cablom == props.pendaftaran.cablom)
})
...
<template>
  <AppLayout>
    <div class="lg:px-40 px-8 text-white">
      ...
      <h2 class="mt-6 mb-10 text-3xl font-semibold">Pendaftaran Lomba: {{ lomba.nama_perlombaan }} ({{
        pendaftaran.cabang_lomba }})</h2>
      <form action="" class="mb-16"
        ...
        <div class="grid grid-cols-6 gap-x-10 gap-y-5">
          ...
          <div class="row-start-4 col-span-2">
            ...
            <a :href="pendaftaran.anggota[0].pivot.berkas_identitas" target="_blank"
              class="text-xs hover:underline mt-1">Lihat File Sebelumnya</a>
          ...
          <div class="row-start-5 col-span-2" v-if="(selectedCablom?.biaya ?? 0) > 0">
            ...
            <a :href="pendaftaran.bukti_pembayaran" target="_blank" class="text-xs hover:underline mt-1">Lihat File
              Sebelumnya</a>
            <InputError :message="form.errors.bukti_pembayaran" />
          </div>
        </div>
        <div class="mt-6" v-for="(anggota, i) in pendaftaran.anggota.slice(1, pendaftaran.anggota.length)"
          :key="anggota.id_peserta">
          <h3 class="text-2xl font-semibold mb-4">Data Anggota {{ i + 1 }}</h3>
          <div class="grid grid-cols-6 gap-x-10 gap-y-5">
            <div class="col-span-2">
              <Label for="kode_anggota" class="mb-2">{{ jenisIdentitas }}</Label>
              <Input id="kode_anggota" v-model="form.kode_anggota[i]" class="bg-silver"
                :placeholder="`Identitas Anggota ${i}`" disabled />
            ...
            <div class="col-span-2">
              ...
              <InputFile id="berkas_anggota" class="bg-silver" v-model="form.berkas_anggota[i]" />
              <a :href="pendaftaran.anggota[i + 1].pivot.berkas_identitas" target="_blank"
                class="text-xs hover:underline mt-1">Lihat File Sebelumnya</a>
